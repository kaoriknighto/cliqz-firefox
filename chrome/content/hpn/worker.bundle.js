  ...
  return {
    ...
    execute: function () {
      ...
      */
      ...
      class UserPk {
        ...
        sign(msg) {
          const promise = new Promise((resolve, reject) => {
            ...
            crypto.subtle.importKey('pkcs8', fromBase64(ppk[1]), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['sign']).then(privateKey => {
              const documentBytes = toUTF8(msg);
              crypto.subtle.sign({ name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, privateKey, documentBytes).then(signatureBuffer => {
        ...
        verify(sig, msg) {
          const promise = new Promise((resolve /* , reject */) => {
            const ppk = privateKeytoKeypair(this.csm.uPK.privateKey);
            crypto.subtle.importKey('spki', fromBase64(ppk[0]), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']).then(publicKey => {
              const signatureBytes = fromHex(sig);
              const documentBytes = toUTF8(msg);
              crypto.subtle.verify({ name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, publicKey, signatureBytes, documentBytes).then(validSignature => {
        ...
        generateKey() {
          const promise = new Promise((resolve, reject) => {
            crypto.subtle.generateKey({
              ...
              hash: { name: 'SHA-256' }
            }, true, ['sign', 'verify']).then(key => crypto.subtle.exportKey('jwk', key.privateKey)).then(key => {
              const returnData = {};
              ...
              return returnData;
            }).then(keys => _http(this.csm.USER_REG).post(JSON.stringify({ pk: keys.publicKeyB64 }))).then(() => resolve({ status: true, privateKey: this.privateKey, publicKey: this.publicKey })).catch(e => reject({ status: e.message }));
          });
          return promise;
        }
      }
      ...
      function toByteArray$1(data, format) {
        if (format === 'hex') {
          return fromHex(data);
        } else if (format === 'b64') {
      ...
      function toArrayBuffer(data, format) {
        return toByteArray$1(data, format).buffer;
      }
      function hash$1(algo, str) {
        let format = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'hex';

        return crypto.subtle.digest(algo, typeof str === 'string' ? toUTF8(str) : str).then(h => fromArrayBuffer(h, format));
      }
      ...
      function encryptAES(data, key, iv) {
        return Promise.all([iv || crypto.getRandomValues(new Uint8Array(12)), typeof key === 'string' ? importAESKey(key) : key]).then(_ref => {
          var _ref2 = _slicedToArray(_ref, 2);

          let _iv = _ref2[0],
              _key = _ref2[1];
          return crypto.subtle.encrypt({ name: 'AES-GCM', iv: _iv }, _key, data).then(encrypted => [fromArrayBuffer(_iv, 'b64'), fromArrayBuffer(encrypted, 'b64')]);
        });
      }
      ...
      function wrapAESKey(aesKey, publicKey) {
        return Promise.resolve(typeof publicKey === 'string' ? importRSAKey(publicKey, true) : publicKey).then(pk => crypto.subtle.wrapKey('raw', aesKey, pk, { name: 'RSA-OAEP', hash: { name: 'SHA-256' } })).then(wrapped => toBase64(wrapped));
      }
      function sha1(s) {
        return hash$1('SHA-1', s);
      }
      ...
      */
      ...
      function parseDSKey(pubKeyB64) {
        // Parse key contents.
        return crypto.subtle.importKey('spki', fromBase64(pubKeyB64), {
          name: 'RSA-OAEP',
          hash: { name: 'SHA-1' }
        }, true, ['encrypt']).then(key => crypto.subtle.exportKey('jwk', key)).then(key => {
      ...
      function unBlindMessage(blindSignedMessage, unBlinder, n) {
        ...
        // s = u*(bs) mod n
        const _us = BigInt.multMod(unBlinder, BigInt.str2bigInt(blindSignedMessage, 16), BigInt.str2bigInt(n, 10));
      ...
      class blindSignContext {
        ...
        getBlinder(e, n) {
          // Calculate blinder.
          // b = r ^ e mod n
          const b = BigInt.powMod(this.blindingNonce, BigInt.str2bigInt(e, 10), BigInt.str2bigInt(n, 10));
          this.blinder = b;
          return b;
        }
        ...
        getUnBlinder(n) {
          // Calculate blinder.
          // b = r ^ e mod n
          const u = BigInt.inverseMod(this.blindingNonce, BigInt.str2bigInt(n, 10));
          this.unblinder = u;
          return u;
        }
        ...
        blindMessage(e, n) {
          ...
          const promise = new Promise(resolve => /* reject */{
            this.hashMessage().then(hashMessage => {
              ...
              const bm = BigInt.multMod(blinder, BigInt.str2bigInt(hashMessage, 16), BigInt.str2bigInt(n, 10));
        ...
        verify() {
          ...
          return new Promise(resolve => /* reject */{
            const messageSigned = BigInt.bigInt2str(BigInt.powMod(BigInt.str2bigInt(this.signedMessage, 10, 0), BigInt.str2bigInt(this.e, 10), BigInt.str2bigInt(this.n, 10)), 10);
            const originalMessage = BigInt.bigInt2str(BigInt.str2bigInt(this.hashedMessage, 16), 10);
      ...
      */
      ...
      class MessageContext {
        ...
        getproxyCoordinator() {
          ...
          this.md5Hash = cachedMD5(this.action);
          const promise = new Promise((resolve, reject) => {
            try {
        ...
        rsaEncrypt(msg) {
          ...
          return crypto.subtle.importKey('spki', fromBase64(publicKey), {
            name: 'RSA-OAEP',
            hash: { name: 'SHA-1' }
          }, false, ['encrypt']).then(key => crypto.subtle.encrypt({ name: 'RSA-OAEP' }, key, toUTF8(msg))).then(toBase64).catch(err => {
        ...
        aesDecrypt(msg) {
          const promise = new Promise((resolve /* , reject */) => {
            ...
            const _iv = this.iv;
            crypto.subtle.importKey('raw', fromHex(hashKey), 'AES-CBC', false, ['decrypt']).then(key => {
        ...
        signKey() {
          const promise = new Promise((resolve, reject) => {
            try {
              ...
              // mE.
              const mI = cachedMD5(this.mE); // replace it with web-crypto md5.
        ...
        sendBlindPayload() {
          const promise = new Promise((resolve, reject) => {
            const payload = createPayloadBlindSignature(this.csm.uPK.publicKeyB64, this.bm1, this.bm2, this.bm3, this.signedData);
            return _http(this.csm.BLIND_SIGNER).post(JSON.stringify(payload)).then(response => {
        ...
        sendMessageProxy() {
          const promise = new Promise((resolve, reject) => {
            const payload = createPayloadProxy(this.csm.uPK.publicKeyB64, this.suPK, this.mP, this.dmC, this.us1, this.us2, this.us3, this.signedMessageProxy);
            return _http(this.proxyCoordinator).post(JSON.stringify(payload)).then(() => resolve(this)).catch(err => {
              reject(err);
            });
          });
          return promise;
      ...
      self.onmessage = e => {
        try {
          ...
          if (msgType === 'telemetry') {
            ...
            parseDSKey(CliqzSecureMessage.dsPK.pubKeyB64).then(keys => {
